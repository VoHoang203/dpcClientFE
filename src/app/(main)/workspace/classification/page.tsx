...
const Classification = () => {
  ...
  const [selectedMember, setSelectedMember] =
    useState<MemberClassification | null>(null);

  const stats = {
    excellent: mockClassifications.filter(
      (m) => m.classification === "excellent"
    ).length,
    good: mockClassifications.filter((m) => m.classification === "good").length,
    complete: mockClassifications.filter(
      (m) => m.classification === "complete"
    ).length,
    incomplete: mockClassifications.filter(
      (m) => m.classification === "incomplete"
    ).length,
    pending: mockClassifications.filter(
      (m) => m.classification === "pending"
    ).length,
  };
            ...
            <div className="space-y-4">
              <div>
                <div className="mb-1 flex justify-between text-sm">
                  ...
                  <span>
                    {stats.excellent} (
                    {Math.round((stats.excellent / total) * 100)}%)
                  </span>
                </div>
                <Progress
                  value={(stats.excellent / total) * 100}
                  className="h-2"
                />
              </div>
              <div>
              ...
              <div>
                <div className="mb-1 flex justify-between text-sm">
                  ...
                  <span>
                    {stats.complete} (
                    {Math.round((stats.complete / total) * 100)}%)
                  </span>
                </div>
                <Progress
                  value={(stats.complete / total) * 100}
                  className="h-2"
                />
              </div>
              <div>
                <div className="mb-1 flex justify-between text-sm">
                  ...
                  <span>
                    {stats.incomplete} (
                    {Math.round((stats.incomplete / total) * 100)}%)
                  </span>
                </div>
                <Progress
                  value={(stats.incomplete / total) * 100}
                  className="h-2"
                />
              </div>
            </div>
